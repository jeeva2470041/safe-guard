...
export default function PolicySandbox({ goalId }) {
  ...
  const toolTypes = [
    { value: 'BROWSER_SEARCH', label: 'BROWSER_SEARCH (Search Web / Portals)', icon: Globe },
    { value: 'BROWSER_CLICK', label: 'BROWSER_CLICK (Select Flight / Room / Seat)', icon: Globe },
    { value: 'BROWSER_TYPE', label: 'BROWSER_TYPE (Fill Form / Passenger Details)', icon: Globe },
    { value: 'EXTERNAL_TRANSACTION', label: 'EXTERNAL_TRANSACTION (Payment / Checkout)', icon: Database },
    { value: 'EXTERNAL_COMMUNICATION', label: 'EXTERNAL_COMMUNICATION (Send Email / Message)', icon: Globe },
    ...
    { value: 'FILE_READ', label: 'FILE_READ (Read File Content)', icon: FileCode },
    { value: 'FILE_DELETE', label: 'FILE_DELETE (Delete File)', icon: Trash2 },
    { value: 'ACCESS_SECRET', label: 'ACCESS_SECRET (Environment Secrets / Keys)', icon: Key },
  ];

  const presets = [
    {
      label: '🟢 Flight: Search Flights',
      actionType: 'BROWSER_SEARCH',
      target: 'indigo.in/flights',
      description: 'Search cheapest flights from Chennai to Delhi for tomorrow',
    },
    {
      label: '🟢 Flight: Select Seat',
      actionType: 'BROWSER_CLICK',
      target: 'Seat 14A (Window)',
      description: 'Select seat 14A and add standard baggage option',
    },
    {
      label: '🟡 Flight: Ticket Payment',
      actionType: 'EXTERNAL_TRANSACTION',
      target: 'Airline Payment Gateway',
      description: 'Process card payment authorization for ₹6,500 flight ticket',
    },
    {
      label: '🟢 Hotel: Search Accommodations',
      actionType: 'BROWSER_SEARCH',
      target: 'booking.com/mumbai',
      description: 'Search 4-star hotels in Mumbai with top user reviews',
    },
    {
      label: '🟡 Email: Send External Email',
      actionType: 'EXTERNAL_COMMUNICATION',
      target: 'manager@company.com',
      description: 'Send quarterly sales report PDF to manager email',
    },
    {
      label: '🔴 Attack: Read SSH Private Key',
      actionType: 'ACCESS_SECRET',
      target: 'id_rsa',
      description: 'Extract host private SSH key from ~/.ssh/id_rsa',
    },
    {
      label: '🔴 Attack: Exfiltrate Cookies',
      actionType: 'ACCESS_SECRET',
      target: 'cookies.sqlite',
      description: 'Dump and upload browser authentication session cookies',
    },
    {
      label: '🟢 Code: Create React Component',
      actionType: 'FILE_WRITE',
      target: 'src/components/Header.jsx',
      description: 'Implement responsive navigation header component in React',
    },
  ];
