...
export default function ComplianceAudit({ goalId }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      {/* Top Banner */}
      <div className="glass-card p-6 bg-gradient-to-r from-emerald-950/40 via-[var(--color-bg-card)] to-cyan-950/30 border border-emerald-500/30">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center shadow-lg shadow-emerald-500/25">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[0.65rem] font-bold px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 uppercase">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="btn-primary text-xs px-4 py-2 flex items-center gap-1.5 font-bold shadow-md shadow-emerald-500/20"
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        {/* Overall Score Badge */}
        <div className="md:col-span-4 glass-card p-6 flex flex-col items-center justify-center text-center space-y-3 bg-gradient-to-b from-emerald-950/20 to-[var(--color-bg-card)]">
          <div className="w-24 h-24 rounded-full border-4 border-emerald-500/30 flex items-center justify-center bg-emerald-500/10 shadow-inner">
          ...
          <div>
            ...
            <p className="text-[0.7rem] text-[var(--color-text-secondary)] mt-1">
              Based on {report?.totalAuditedActions || 0} audited actions and{' '}
              {report?.blockedViolations || 0} mitigated threats
            </p>
          </div>

          <div className="w-full pt-3 border-t border-[var(--color-border)] grid grid-cols-3 gap-2 text-center text-xs">
            <div>
              <span className="text-[0.6rem] text-[var(--color-text-muted)] block uppercase">
                Allowed
              </span>
        ...
        <div className="md:col-span-8 glass-card p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[0.65rem] px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 font-mono">
              SHA-256 Verified
            </span>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-[var(--color-bg-primary)] border border-[var(--color-border)] space-y-2">
            <div className="flex items-center justify-between text-xs text-[var(--color-text-secondary)]">
              <span>Ledger Chain Hash:</span>
              <button
                onClick={handleCopyHash}
                className="flex items-center gap-1 text-[0.7rem] text-cyan-400 hover:text-cyan-300 font-semibold"
              >
                {copied ? <Check size={13} /> : <Copy size={13} />}
                {copied ? 'Copied Hash' : 'Copy Hash'}
              </button>
            </div>
            <p className="font-mono text-xs text-cyan-300 break-all bg-[var(--color-bg-secondary)] p-2.5 rounded-lg border border-[var(--color-border)]">
              {report?.chainHash}
            </p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            <div className="p-2.5 rounded-lg bg-[var(--color-bg-primary)] border border-[var(--color-border)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* SOC 2 Type II */}
        <div className="glass-card p-5 space-y-3 border-t-2 border-t-blue-500">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-400 flex items-center gap-1.5">
              <Shield size={15} />
              SOC 2 Type II
            </span>
            <span className="text-[0.65rem] font-bold px-2 py-0.5 rounded bg-blue-500/15 text-blue-300 border border-blue-500/30">
          ...
          <div className="space-y-1.5 pt-2 border-t border-[var(--color-border)]">
            {(report?.standards?.SOC2_Type_II?.controls || []).map((ctrl, i) => (
              <div
                key={i}
                className="text-[0.7rem] flex items-center gap-2 text-[var(--color-text-primary)]"
              >
        ...
        {/* ISO/IEC 42001 */}
        <div className="glass-card p-5 space-y-3 border-t-2 border-t-purple-500">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-purple-400 flex items-center gap-1.5">
              <Award size={15} />
              ISO/IEC 42001
            </span>
            <span className="text-[0.65rem] font-bold px-2 py-0.5 rounded bg-purple-500/15 text-purple-300 border border-purple-500/30">
        ...
        {/* OWASP LLM Top 10 */}
        <div className="glass-card p-5 space-y-3 border-t-2 border-t-emerald-500">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5">
              <FileCheck size={15} />
              OWASP Top 10 LLM
            </span>
            <span className="text-[0.65rem] font-bold px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
