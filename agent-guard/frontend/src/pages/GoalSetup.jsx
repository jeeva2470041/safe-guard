...
export default function GoalSetup({ onStart, sessionStatus, onStatusChange }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-[1140px] mx-auto w-full px-6 py-8 space-y-8">
        {/* Hero Section */}
        <div className="text-center space-y-2">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500 via-blue-600 to-indigo-600 flex items-center justify-center mx-auto shadow-lg shadow-cyan-500/25 animate-pulse-glow">
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-[0.6rem] font-bold px-2 py-0.5 rounded border uppercase ${sc.badgeColor}`}>
                        {sc.badge}
                      </span>
                    </div>
        ...
        <div className="glass-card p-6 space-y-5">
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-xl px-4 py-3 text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 resize-none transition-all font-sans leading-relaxed shadow-inner"
          ...
          {!goalPolicy && (
            <div className="space-y-2.5">
              <button
                ...
                className="btn-primary w-full flex items-center justify-center gap-2 py-3.5 text-xs font-bold tracking-wider shadow-lg shadow-cyan-500/20 disabled:opacity-50"
          ...
          {goalPolicy && (
            <div className="p-4 rounded-xl bg-blue-950/30 border border-cyan-500/40 space-y-3 animate-fade-in-up">
              <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(goalPolicy.allowedScope || []).map((scope, idx) => (
                    <span
                      key={idx}
                      className="text-[0.65rem] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-mono"
                    >
                      ✓ {scope}
                    </span>
                  ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {goalPolicy.restrictedScope.map((scope, idx) => (
                      <span
                        key={idx}
                        className="text-[0.65rem] px-2 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20 font-mono"
                      >
                        🚫 {scope}
                      </span>
                    ))}
              ...
              <div className="grid grid-cols-1 gap-3 pt-2">
                <button
                  onClick={handleAcceptAndMonitor}
                  disabled={loading}
                  className="btn-primary flex items-center justify-center gap-2 py-3 text-xs font-bold"
                  style={{ background: 'linear-gradient(135deg, #0ea5e9, #6366f1)', border: 'none' }}
                >
                  {loading ? (
                    'Activating Monitor...'
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    disabled={loading}
                    className="btn-primary flex items-center justify-center gap-2 py-2.5 text-xs bg-emerald-600 hover:bg-emerald-500 border-none font-semibold"
                  >
                    <Zap size={14} />
                    START AGENT RUN
                  </button>
                  ...
                  <button
                    ...
                    className="btn-secondary py-2.5 text-xs text-[var(--color-text-muted)] hover:text-white"
                  >
                    MODIFY PROMPT
                  </button>
                </div>
