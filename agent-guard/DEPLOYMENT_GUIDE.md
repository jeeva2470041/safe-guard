...
1. On your local machine, set the environment variable:
   - **Windows (PowerShell)**:
     ```powershell
     $env:AGENT_GUARD_API="https://agent-guard-backend.onrender.com/api/agent/intercept"
     $env:AGENT_GUARD_SESSION_API="https://agent-guard-backend.onrender.com/api/agent/session/start"
     ```
   - **Linux / macOS**:
     ```bash
